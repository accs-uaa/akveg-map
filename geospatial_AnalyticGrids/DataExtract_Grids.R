...
grid_csv = paste(drive,
                 root_folder,
                 'Data/analyses/gridMinor_Selected.csv',
                 sep ='/')
...
grid_list = as.vector(grid_table['minor'])
...
for (grid in grid_list) {
  ...
  grid_raster = paste(grid_folder, '/', grid, '.tif', sep = '')
  
  # Convert raster to spatial dataframe of points
  grid_points = rasterToPoints(grid_raster, spatial=TRUE)
  
  # Identify grid name and predictor folders
  major_grid = substr(grid, start = 1, stop = 2)
  ...
  climate_grid = paste(topography_folder, major_grid, sep = '/')
  ...
  grid_extracted = data.frame(grid_points@data, extract(predictor_stack, grid_points))
