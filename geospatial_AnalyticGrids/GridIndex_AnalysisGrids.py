...
# Set data folder
data_folder = os.path.join(root_folder, 'Projects/VegetationEcology/AKVEG_QuantitativeMap/Project_GIS')

# Set arcpy working environment
arcpy.env.workspace = os.path.join(data_folder, 'BeringiaVegetation.gdb')
...
major_kwargs = {'tile_name': 'PageName',
                ...
                'output_geodatabase': os.path.join(data_folder, 'Analysis_GridMajor.gdb')
                }
minor_kwargs = {'tile_name': 'Name',
                'distance': '10 Meters',
                'input_array': minor_inputs,
                'output_geodatabase': os.path.join(data_folder, 'Analysis_GridMinor.gdb')
                }

# Create the major and minor grid indices
arcpy_geoprocessing(create_grid_indices, **grid_kwargs)
