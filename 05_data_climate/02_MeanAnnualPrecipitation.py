# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Calculate mean annual precipitation for 2010-2020
# Author: Timm Nawrocki
# Last Updated: 2026-01-18
# Usage: Must be executed in a Python 3.12+ installation.
# Description: "Calculate mean annual precipitation for 2006-2015" calculates the mean annual precipitation from all months for years 2006-2015. The primary data are the SNAP Alaska-Yukon 2km data with the included portion of the Northwest Territories interpolated by geographic nearest neighbors.
# ---------------------------------------------------------------------------
...
climate_folder = os.path.join(data_folder, 'unprocessed/pr_2km_TS408')
...
precip_intermediate = os.path.join(processed_folder, 'Precipitation_2006_2015_2km_3338.tif')
precip_filled = os.path.join(processed_folder, 'Precipitation_2006_2015_2km_3338_Filled.tif')

# Define output datasets
precip_output = os.path.join(processed_folder, 'Precipitation_2006_2015_10m_3338.tif')

# Define month and property values
climate_property = 'pr_total_mm_cru_ts408_historical'
...
with rasterio.open(precip_intermediate, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        out_block = np.round((out_block/len(years)), 0)
...
climate_raster = rasterio.open(precip_filled)
...
with rasterio.open(precip_output, 'w', **output_profile) as dst:
    # Find number of raster blocks
    window_list = []
    for block_index, window in dst.block_windows(1):
        window_list.append(window)
...
os.remove(precip_intermediate)
os.remove(precip_filled)
