# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Figure 7. Temperate map results
# Author: Timm Nawrocki, Alaska Center for Conservation Science
# Last Updated: 2025-07-20
# Usage: Script should be executed in R 4.4.3+.
# Description: "Figure 7. Temperate map results" creates a map figure for publication that shows three select foliar cover maps in the Alaska Pacific region.
# ---------------------------------------------------------------------------
...
# Define input files
imagery_input = path(output_folder, 'data', 'Figure7_Imagery_0.5m_3338.tif')
rubspe_input = path(raster_folder, 'rubspe/rubspe_10m_3338.tif')
picsit_input = path(raster_folder, 'picsit/picsit_10m_3338.tif')
wetsed_input = path(raster_folder, 'wetsed/wetsed_10m_3338.tif')
photo_input = path(output_folder, 'data', 'Figure7f_Salmonberry.jpg')
...
# Define output files
figure_output = path(output_folder, 'Figure7_Temperate_Map_Results.jpg')

#### CREATE DETAIL PLOTS
####------------------------------

# Define center coordinates and extent (approximately 1:24,000 scale)
center_lon = -148.0130
center_lat = 59.9532
...
lon_breaks = seq(floor(coords_4326$xmin * 100) / 100, ceiling(coords_4326$xmax * 100) / 100, by = 0.03)
...
# Prepare rubspe
rubspe_raster = rast(rubspe_input)
rubspe_crop = crop(rubspe_raster, bounding_3338)
rubspe_crop[rubspe_crop < 5] = NA
...
# Prepare picsit
picsit_raster = rast(picsit_input)
picsit_crop = crop(picsit_raster, bounding_3338)
picsit_crop[picsit_crop < 5] = NA
...
base_plot = ggplot() +
  geom_spatraster_rgb(data = imagery_crop, r = 3, g = 2, b = 1) +
  coord_sf(
    crs = st_crs(3338),
    xlim = x_limits,
    ylim = y_limits,
...
rubspe_plot = ggplot() +
  geom_spatraster_rgb(data = imagery_crop, r = 3, g = 2, b = 1) +
  geom_spatraster(data = rubspe_crop) +
...
picsit_plot = ggplot() +
  geom_spatraster_rgb(data = imagery_crop, r = 3, g = 2, b = 1) +
  geom_spatraster(data = picsit_crop) +
  scale_fill_gradientn(
    colors = custom_palette,
    limits = c(3, 50),
    oob = scales::squish,
...
locator_plot = ggplot() +
  ...
  ggtitle('e. Detail location (red point)') +
  theme_minimal() +
  theme(
    axis.title = element_blank(),
    axis.text = element_text(size = 14),
...
image_plot = image_ggplot(photo_data, interpolate = FALSE) +
  theme_minimal() +
  ggtitle('f. Sitka spruce and salmonberry (photo)') +
  theme(
    plot.title = element_text(size = 18),
...
# Create merged plot
combine_plot = ggarrange(base_plot,
                         rubspe_plot,
                         picsit_plot,
                         wetsed_plot,
                         locator_plot,
