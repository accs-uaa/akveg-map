...
warmth_15km = os.path.join(processed_15km, 'SummerWarmth_MeanAnnual_15km_2000-2015.tif')
...
property_15km = 'tas_mean_C_CRU-TS40_historical'
...
if arcpy.Exists(warmth_15km) == 0:
    print('Calculating 15km mean annual summer warmth index...')
    arcpy_geoprocessing(calculate_climate_mean, **kwargs_15km)
    print('----------')
else:
    print('Mean annual summer warmth index at 15 km resolution already exists...')
    print('----------')

# Define input and output arrays to combine climate resolutions
climate_resolution_inputs = [warmth_2km, warmth_15km]
climate_resolution_outputs = [warmth_combined]
...
if arcpy.Exists(warmth_combined) == 0:
...
for grid in grids:
    ...
    if arcpy.Exists(climate_grid) == 0:
        ...
        climate_grid_inputs = [grid, warmth_combined]
