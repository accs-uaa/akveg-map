# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare Class Data - Salix pulchra
# Author: Timm Nawrocki
# Created on: 2020-05-16
# Usage: Must be executed in R 4.0.0+.
# Description: "Prepare Class Data - Salix pulchra" prepares the map class data for statistical modeling.
# ---------------------------------------------------------------------------
...
presence_sites = species_data %>%
  filter(nameAccepted == 'Salix pulchra') %>%
...
absence_sites = site_data['siteCode'] %>%
  ...
  mutate(nameAccepted = 'Salix pulchra') %>%
  mutate(genus = 'Salix') %>%
  mutate(coverTotal = 0) %>%
  mutate(zero = 0)
...
output_csv = paste(data_folder, 'species_data/mapClass_SalixPulchra.csv', sep = '/')
