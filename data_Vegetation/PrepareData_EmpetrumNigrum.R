...
presence_data = cover_data %>%
  filter(name_accepted == 'Empetrum nigrum' |
...
presence_data = presence_data %>%
  ...
  mutate(name_accepted = 'Emeptrum nigrum') %>%
...
absence_data = absence_data %>%
  mutate(name_accepted = 'Empetrum nigrum') %>%
  mutate(genus = 'Empetrum') %>%
  mutate(cover = 0) %>%
  mutate(zero = 0)
...
output_csv = paste(data_folder, 'species_data/mapClass_empnig.csv', sep = '/')
