...
presence_data = cover_data %>%
  filter(name_accepted == 'Betula nana' |
           name_accepted == 'Betula nana ssp. exilis' |
           name_accepted == 'Betula glandulosa') %>%
...
presence_data = presence_data %>%
  ...
  mutate(name_accepted = 'Betula Shrubs') %>%
  mutate(genus = 'Betula') %>%
...
output_csv = paste(data_folder, 'species_data/mapClass_betshr.csv', sep = '/')
