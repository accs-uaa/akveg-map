# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare Class Data - Hylocomium splendens
# Author: Timm Nawrocki
# Created on: 2020-05-16
# Usage: Must be executed in R 4.0.0+.
# Description: "Prepare Class Data - Hylocomium splendens" prepares the map class data for statistical modeling.
# ---------------------------------------------------------------------------
...
presence_sites = species_data %>%
  filter(nameAccepted == 'Hylocomium splendens') %>%
...
absence_sites = site_data['siteCode'] %>%
  ...
  mutate(nameAccepted = 'Hylocomium splendens') %>%
  mutate(genus = 'Hylocomium splendens') %>%
  mutate(coverTotal = 0) %>%
  mutate(zero = 0)
...
output_csv = paste(data_folder, 'species_data/mapClass_HylocomiumSplendens.csv', sep = '/')
