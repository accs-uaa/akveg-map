# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare Class Data - Salix fuscescens
# Author: Timm Nawrocki
# Created on: 2020-05-16
# Usage: Must be executed in R 4.0.0+.
# Description: "Prepare Class Data - Salix fuscescens" prepares the map class data for statistical modeling.
# ---------------------------------------------------------------------------
...
presence_sites = species_data %>%
  filter(nameAccepted == 'Salix fuscescens') %>%
...
absence_sites = site_data['siteCode'] %>%
  ...
  mutate(nameAccepted = 'Salix fuscescens') %>%
  mutate(genus = 'Salix') %>%
  mutate(coverTotal = 0) %>%
  mutate(zero = 0)
...
output_csv = paste(data_folder, 'species_data/mapClass_SalixFuscescens.csv', sep = '/')
