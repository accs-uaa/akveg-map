...
# Description: "Prepare Class Data - Picea mariana" prepares the Picea mariana data for statistical modeling.
# ---------------------------------------------------------------------------
...
presence_sites = species_data %>%
  filter(nameAccepted == 'Picea mariana') %>%
...
absence_sites = site_data['siteCode'] %>%
  ...
  mutate(nameAccepted = 'Picea mariana') %>%
  mutate(genus = 'Picea') %>%
  mutate(coverTotal = 0) %>%
  mutate(zero = 0)
...
output_csv = paste(data_folder, 'species_data/mapClass_PiceaMariana.csv', sep = '/')
