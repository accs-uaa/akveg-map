...
output_folder = os.path.join(data_folder, 'Canada_Composite_DTM_10m')

# Define input files
usgs_30m_file = os.path.join(data_folder, 'USGS_3DEP_30m/processed', 'USGS_3DEP_30m_3338.tif')
usgs_60m_file = os.path.join(data_folder, 'USGS_3DEP_60m/processed', 'USGS_3DEP_60m_3338.tif')
arctic_dem_file = os.path.join(data_folder, 'Arctic_DEM_10m/processed', 'Arctic_DEM_10m_3338.tif')
#area_file = os.path.join(data_folder, 'area_test.tif')
area_file = os.path.join(data_folder, 'Canada_DEM_MapDomain_10m_3338.tif')

# Define output files
output_file = os.path.join(output_folder, 'processed', 'Canada_Composite_10m_3338.tif')
...
input_profile = arctic_raster.profile.copy()
...
with rasterio.open(output_file, 'w', **input_profile, BIGTIFF='YES') as dst:
    ...
    progress = 0
    for block_index, window in area_raster.block_windows(1):
        arctic_block = arctic_raster.read(window=window,
                                          masked=False)
        usgs_30m_block = usgs_30m_raster.read(window=window,
                                              masked=False)
        usgs_60m_block = usgs_60m_raster.read(window=window,
                                              masked=False)
        area_block = area_raster.read(window=window,
                                      masked=False)
        # Set values where usgs 30 m block is nodata
        raster_block = np.where((usgs_30m_block == nodata) & (usgs_60m_block != nodata) & (arctic_block != nodata), (arctic_block + (usgs_60m_block * 3)) / 4, arctic_block)
        # Set values where usgs 60 m block is nodata
        raster_block = np.where((usgs_30m_block != nodata) & (arctic_block != nodata), (arctic_block + (usgs_30m_block * 3)) / 4, raster_block)
